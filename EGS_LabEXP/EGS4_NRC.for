...
c 
      ...
      tlim = 0.
c
cnew      call timed(tevent)
cnew      call timex(totim)
cnew      write(*,*) ' totim = ' ,totim,' tevent = ',
cnew     a                                         tevent,' tlim =  ' , tlim
c
c-----  loop over sources
c
...
c---- end of event loop
c
cnico         if( nevent - ncases ) 10,20,20
         if( (nevent - ncases).lt.0 ) goto 10
...
c
cnew      call datime(idm1,idm2)

cnico cambia la chiamata a datime 31/3/20
...
cnico       do 12 k = 1,numcellx
cnico          do 12 j = 1,numcelly
       do k = 1,numcellx
          do j = 1,numcelly
		iu1 = k
		iu2 = int((j+k)/2)
...
c-------
c
cnico          if( nplan - k ) 20,20,10
          if((nplan - k).gt.0) goto 10
c
   20     continue
ccc	else
c
...
c
      eireal = ein
c
cnew      call hfill (100,eireal,0.,1.)
c
      totein  =  totein + ein
c
...
c
     ...
     +	             * rszsqinv(nactsx)) - 1).le.0) goto 21
cnico     +          * rszsqinv(nactsx)) - 1 )    21,21,99
c
...
c
     ...
     +	                            * rsxsqinv(nactsx)
     +         + ((yi0-sy_cent(nactsx))*(yi0-sy_cent(nactsx))) 
     +	             * rsysqinv(nactsx))- 1).le.0) goto 22
cnico     +	       * rsysqinv(nactsx)) - 1 )    22,22,98
c
...
c
     ...
     +	                         * rszsqinv(nactsx)
     +         + ((yi0-sy_cent(nactsx))*(yi0-sy_cent(nactsx)))
     +	          * rsysqinv(nactsx)) - 1 ).le.0)  goto 222
cnico     +	    * rsysqinv(nactsx)) - 1 )    222,222,97
c
...
c
      ...
      zin      =   zi0
c
cnew      call hfill (101,xin,0.,1.)
cnew      call hfill (102,yin,0.,1.)
cnew      call hfill (103,zin,0.,1.)
cnew      call hfill (200,xin,yin,1.)
cnew      call hfill (201,xin,zin,1.)
cnew      call hfill (202,yin,zin,1.)
cc
...
c
      ...
      vin = vin / dnorm
c
cnew      call hfill (110,uin,0.,1.)
cnew      call hfill (111,vin,0.,1.)
cnew      call hfill (112,win,0.,1.)
c
c-----------------------------------------------------------
c
...
c
      edepr = edep
cnew      if (edepr.ne.0.) call hfill (120,edepr,0.,1.)
c
...
c
          edepr=edep
cnew          if (edepr.ne.0.) call hfill (121,edepr,0.,1.)
c
...
c
          edepr=edep
cnew          if (edepr.ne.0.) call hfill (122,edepr,0.,1.)        
c
      else
c
          toteew = toteew + edep 
...
cnico                if( abs(dstrack)-steplim ) 20,10,10
                if((abs(dstrack)-steplim).ge.0) goto 10
   20        continue
c
...
c
      ...
      multiplicity = 0
c
cnew      if (biosource.gt.0.) call hfill(123,biosource,0.,1.)
cnew      if (elost.gt.0.) call hfill(124,elost,0.,1.)
c
      epixel=0.0
      xpixel=0.0
      ypixel=0.0
